import { hasOwn, invokeArrayFns, isString, ShapeFlags } from "@vue3/shared";
...
export function createRenderer(options) {
  ...
  // 核心diff算法
  // 如果不传入key , 那么就会直接对比标签是否相同, 如果相同就复用, 不相同就替换
  // 所以一定要传入唯一的key
    ...
    const effect = new ReactiveEffect(
      () => {
        ...
        if (instance.mounted) {
          ...
          //更新
          //在这一步 要获取parent 来实现provide和inject方法
          const subTree = render.call(instance.proxy, instance.proxy);
          // 这里的instance 就是subTree的parent
